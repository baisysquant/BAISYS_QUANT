...
def main():
    try:
        ...
        if zero_axis_key in details:
            ...
            if len(df) >= 2:
                dif_above = df["DIF_12269"].iloc[-1] > 0 if "DIF_12269" in df.columns else False
                dea_above = df["DEA_12269"].iloc[-1] > 0 if "DEA_12269" in df.columns else False
                if dif_above and dea_above:
                    print(f"   Conditions met: DIF>0 and DEA>0 -> score should equal weight")
                    if zero_score == zero_weight:
                        print(f"   ✓ Score correctly equals weight for zero axis condition")
                    else:
                        print(f"   ⚠ Score ({zero_score}) does not equal weight ({zero_weight})")
                else:
        ...
        if processor_has_weights and processor_has_thresholds:
            ...
            print("   ✓ SignalManager can access config values")
        else:
