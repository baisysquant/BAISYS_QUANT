...
class StockAnalysisCoordinator:
    ...
    def __init__(self, config_file: str = "config.ini"):
        ...
        # 初始化缓存管理器
        cache_dir = os.path.join(self.config.TEMP_DATA_DIRECTORY, "cache")
        self.cache_manager = UnifiedCacheManager(
            cache_dir=cache_dir,
            default_strategy=CacheStrategy.DAILY,
            auto_cleanup=True
        )
        ...
        self.logger.info(f"[INFO] 股票分析协调器初始化完成 - {datetime.now()}")
    ...
    def run(self) -> None:
        ...
        self.logger.info(f"[INFO] 识别的业务日期(最后一个交易日)为: {self.today_str}")
        ...
        try:
            ...
            # 步骤5：处理技术指标信号
            ta_signals = self.analysis_service.process_technical_signals(
                stock_codes_prefixed, hist_df, spot_data
            )
            ...
            # 步骤7：处理均线突破数据
            processed_xstp_df = self.analysis_service.process_xstp_and_filter(
                raw_data, spot_data
            )
            processed_xstp_df = self._filter_by_universe(
                processed_xstp_df, set(stock_codes_pure)
            )
            ...
            # 步骤9：合并和处理数据
            consolidated_report = self.data_processing.consolidate_data(
                processed_data, stock_codes_pure
            )
            ...
            # 步骤11：剔除弱势股
            consolidated_report = self.analysis_service.filter_weak_stocks(
                consolidated_report
            )
            ...
            # 步骤13：同步到数据库
            self._sync_results_to_database(
                consolidated_report, industry_df, raw_data
            )
    ...
    def _filter_raw_data(
        ...
        for key in keys_to_filter:
            if key in raw_data:
                raw_data[key] = self._filter_by_universe(
                    raw_data.get(key, pd.DataFrame()), universe_set
                )
