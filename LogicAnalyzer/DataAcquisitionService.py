...
from UtilsManager.Exceptions import (
    DataFetchError,
...
class DataAcquisitionService:
    ...
    def __init__(self, config, calendar_mgr, logger, cache_manager):
        ...
        self.data_fetcher = DataFetcher(config, calendar_mgr, logger)
    ...
    def get_all_raw_data(self, today_str: str) -> Dict[str, pd.DataFrame]:
        ...
        def fetch_data_source_worker(item):
            ...
            try:
                ...
                if not df.empty:
                    ...
                    if is_valid:
                        self.logger.info(f"  - ✓ {desc}: {len(df)} 条记录")
                        return key, df
                    else:
        ...
        futures = {
            executor2.submit(fetch_data_source_worker, item): item
        ...
        for future in as_completed(futures):
            try:
                ...
                self.logger.error(f"获取数据源时发生异常: {e}")
        ...
        futures = {
            executor3.submit(fetch_xstp_worker, config): config
            for config in xstp_configs
        }
        ...
        for future in as_completed(futures):
            try:
                ...
                self.logger.error(f"获取均线突破数据时发生异常: {e}")
        ...
        try:
            ...
            if not main_cost_df.empty:
                ...
                if ColumnNames.AKSHARE_CODE_RAW in main_cost_df.columns and ColumnNames.STOCK_CODE not in main_cost_df.columns:
                ...
                if is_valid:
                    ...
                    self.logger.info(f"  - ✓ 主力成本数据: {len(main_cost_df)} 条记录")
    ...
    def _fetch_industry_data(self, today_str: str) -> pd.DataFrame:
        ...
        if os.path.exists(industry_info_path):
            try:
                self.logger.info(f"  - 发现本地缓存文件，正在读取: {industry_info_filename}")
                ...
                if not industry_board_df.empty:
                    ...
                    if is_valid:
                        self.logger.info(f"  - ✓ 缓存数据有效: {len(industry_board_df)} 个板块")
                    else:
                        self.logger.warning(
            ...
            self.logger.info("本地无缓存，正在通过接口获取")
            try:
                ...
                if not industry_board_df.empty:
                    ...
                    if is_valid:
                        try:
                            ...
                            self.logger.info(f"  - ✓ 获取成功并已保存: {len(industry_board_df)} 个板块")
                        except Exception as e:
    ...
    def _get_top_industry_constituents(self, industry_board_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if cached_df is not None:
            self.logger.debug(f"命中板块成分股缓存: {len(cached_df)}条记录")
            return cached_df
        ...
        def fetch_worker(row):
            try:
                if isinstance(row, pd.Series):
                    ...
                    self.logger.error(f"[ERROR] 无法识别的数据类型: {type(row)}")
                    return None
                
                self.logger.info(f" - 正在获取板块成分股: {industry_name}")
                ...
                if constituents_df is not None and not constituents_df.empty:
                    ...
                    if "股票代码" in constituents_df.columns:
                        ...
                        constituents_df["股票代码"] = constituents_df["股票代码"].apply(
                            CodeNormalizer.normalize
                        )
        ...
        if results:
            ...
            if valid_results:
                ...
                self.logger.info(f"板块成分股数据已缓存: {len(final_df)}条记录")
                return final_df
