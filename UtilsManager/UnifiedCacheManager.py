...
class CacheEntry:
    ...
    def is_expired(self, strategy: CacheConfig) -> bool:
        ...
        if strategy.strategy == CacheStrategy.DAILY:
            ...
            created_date = datetime.fromtimestamp(self.created_at).date()
            today = datetime.now().date()
            return (created_date.year != today.year or 
...
class UnifiedCacheManager:
    ...
    def _load_metadata(self, key: str) -> Optional[CacheEntry]:
        """加载缓存元数据"""
        meta_path = self._get_metadata_path(key)
    ...
    def save_dataframe(
        ...
        if df is None or df.empty:
            self.logger.warning(f"跳过空DataFrame缓存: {name}")
            return False
        ...
        try:
            ...
            if estimated_size > config.max_size_mb * 1024 * 1024:
                self.logger.warning(
                    f"缓存文件超过大小限制 ({config.max_size_mb}MB): {name}"
                )
                return False
            ...
            self.stats["writes"] += 1
            self.logger.debug(f"缓存已保存: {name} ({len(df)}行, {file_size/1024:.1f}KB)")
            return True
            
        except Exception as e:
            self.logger.error(f"保存缓存失败 [{name}]: {e}")
            return False
    ...
    def load_dataframe(
        ...
        if entry and entry.is_expired(config):
            self.logger.debug(f"缓存已过期，删除: {name}")
        ...
        try:
            ...
            if config.validate_on_load:
                if df.empty:
                    self.logger.warning(f"缓存数据为空: {name}")
                    self._remove_cache(key)
                    self.stats["misses"] += 1
                    return None
                
                if validate_func and not validate_func(df):
                    self.logger.warning(f"缓存数据验证失败: {name}")
                    self._remove_cache(key)
                    self.stats["misses"] += 1
                    return None
            
            self.stats["hits"] += 1
            self.logger.debug(f"缓存命中: {name} ({len(df)}行)")
            return df
            
        except Exception as e:
            self.logger.warning(f"加载缓存失败 [{name}]: {e}，将重新获取")
            self._remove_cache(key)
            self.stats["misses"] += 1
            return None
    ...
    def save_text(
        ...
        try:
            ...
            self.logger.error(f"保存文本缓存失败 [{name}]: {e}")
            return False
    ...
    def load_text(
        ...
        try:
            ...
            self.logger.warning(f"加载文本缓存失败 [{name}]: {e}")
            self._remove_cache(key)
            self.stats["misses"] += 1
            return None
    ...
    def invalidate(self, name: str, params: Dict = None):
        ...
        key = self._generate_cache_key(name, params)
        self._remove_cache(key)
        self.logger.info(f"缓存已手动清除: {name}")
    ...
    def clear_all(self):
        ...
        self.logger.info(f"所有缓存已清除: {self.cache_dir}")
    ...
    def cleanup_expired(self):
        ...
        for meta_file in self.cache_dir.glob("*.meta.json"):
            try:
                ...
                self.logger.warning(f"清理缓存失败 [{meta_file.name}]: {e}")
        
        if cleaned_count > 0:
            self.stats["cleanups"] += 1
            self.logger.info(f"已清理 {cleaned_count} 个过期缓存")
        ...
        for path in [cache_path, meta_path]:
            if path.exists():
                try:
                    ...
                    self.logger.warning(f"删除缓存文件失败 [{path.name}]: {e}")
    
    def get_stats(self) -> Dict:
        """获取缓存统计信息"""
        total_files = len(list(self.cache_dir.glob("*")))
        cache_files = len(list(self.cache_dir.glob("*.csv"))) + \
                     len(list(self.cache_dir.glob("*.txt")))
