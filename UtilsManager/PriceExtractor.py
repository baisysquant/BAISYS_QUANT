...
class PriceExtractor:
    ...
    def extract_latest_prices(
        hist_df: pd.DataFrame, date_col: str | None = None, price_col: str | None = None, code_col: str | None = None
    ) -> pd.DataFrame:
        """
        从K线数据中提取每个股票的最新收盘价
    ...
    @staticmethod
    def _detect_column(columns: pd.Index, candidates: list[str]) -> str | None:
        """
        从候选列表中检测第一个存在的列名
    ...
    @staticmethod
    def extract_with_fallback(hist_df: pd.DataFrame, fallback_price: float = 0.0) -> pd.DataFrame:
        """
        提取最新价格，失败时返回默认值
        ...
        try:
            ...
            if code_col:
                ...
                result_df = pd.DataFrame({"股票代码": normalized_codes, "最新价": fallback_price})
                return result_df
            else:
                return pd.DataFrame(columns=["股票代码", "最新价"])
